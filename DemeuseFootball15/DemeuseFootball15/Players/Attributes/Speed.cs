...
{
    public class Speed : PlayerAttribute
    {
        [PotentialProperty]
        ...
        public override void Create(Player player, IDiceShaker shaker, IDiceAttribute diceAttribute)
        {
            ...
            {
                _strengthModifier = shaker.RandomRoll(-.05, 0);
            }
            else if (strength > 60 && strength <= 75)
            {
                _strengthModifier = shaker.RandomRoll(0, .09);
            }
            ...
            {
                _twitchModifier = shaker.RandomRoll(.09, .25);
            }
